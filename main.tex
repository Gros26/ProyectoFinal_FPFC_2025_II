...
\author{
    Sebastian Oviedo \\ \texttt{xxxx} \\
    Juan Moreno \\ \texttt{xxxx} \\
    Weis Paul \\ \texttt{2513207} \\
    Grosman Klein García Valencia \\ \texttt{2340247}
}
\date{Diciembre 2025}
...
def itinerariosAire(vuelos: List[Vuelo], aeropuertos: List[Aeropuerto]): 
  ...
  def distancia(org: Aeropuerto, dst: Aeropuerto): Double = {
    Math.sqrt(Math.pow(org.X - dst.X, 2) + Math.pow(org.Y - dst.Y, 2))
  }

  def tiempoAireTotal(it: Itinerario): Double = {
    it.map { v =>
      val aeroOrg = aeropuertos.find(_.Cod == v.Org).get
      val aeroDst = aeropuertos.find(_.Cod == v.Dst).get
      distancia(aeroOrg, aeroDst)
    }.sum
  }

  (cod1: String, cod2: String) => {
    val todos = generarItinerarios(cod1, cod2)
    val ordenados = todos.sortBy(tiempoAireTotal)
    ordenados.take(3)
  }
}
\end{minted}
...
def itinerariosAirePar(vuelos: List[Vuelo], aeropuertos: List[Aeropuerto]): 
  ...
  // Utiliza la versión paralela para generar el espacio de búsqueda
  val generarItinerariosPar = itinerariosPar(vuelos, aeropuertos)

  // ... (definición de distancia y tiempoAireTotal idéntica a secuencial) ...

  (cod1: String, cod2: String) => {
    val todos = generarItinerariosPar(cod1, cod2)

    // Evaluación paralela de la función de costo (floating point)
    val ordenados = todos.par
      .map(it => (tiempoAireTotal(it), it))
      .toList
      .sortBy(_._1)
      .map(_._2)
